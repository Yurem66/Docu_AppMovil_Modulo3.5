# Documentación Oficial - AutoElite Motors & Rentals (Módulo 3.5)

Bienvenido a la documentación técnica formal de la aplicación móvil **AutoElite Motors & Rentals**, desarrollada para el Módulo 3.5 (Desarrollo de Aplicaciones Móviles) del 3.º Año de Bachillerato Técnico Vocacional en Desarrollo de Software (3DSA 2026), Complejo Educativo Delgado.

Este repositorio sigue el estándar internacional **Docs-as-Code**, estructurando la arquitectura, guías de instalación, especificación de API, manuales de usuario y directivas de seguridad directamente en Markdown.

---

## Información del Proyecto
* **Proyecto:** AutoElite Motors & Rentals
* **Módulo:** Módulo 3.5 — Desarrollo en React Native
* **Institución:** Complejo Educativo Delgado
* **Año / Sección:** 3.º Técnico 2026 (3DSA)
* **Integrantes del Equipo:**
  * Victor Daniel Rivera Lopez
  * Yurem Alessandro Membreño Rivas

---

## Índice de Documentación Técnica

Navega a través de los diferentes módulos de la documentación oficial:

- **Arquitectura y Modelado UML:**
  - [Ver Diagrama de Casos de Uso](docs/casos_de_uso.md)
  - [Ver Diagrama de Secuencia](docs/secuencia.md)
-  **Configuración e Infraestructura:**
  - [Ver Guía de Instalación Móvil](docs/instalacion_movil.md)
  - [Ver Especificación de API REST y Servicios](docs/api_endpoints.md)
-  **Guías y Políticas:**
  - [Ver Manual de Usuario Móvil](docs/manual_usuario.md)
  - [Ver Políticas de Seguridad Móvil](docs/seguridad.md)
- **Historial de Entregas:**
  - [Ver Historial de Cambios (CHANGELOG)](CHANGELOG.md)
